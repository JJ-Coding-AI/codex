...
const logBlackElem = document.getElementById('log-black');
const logWhiteElem = document.getElementById('log-white');
...
function appendLog(text, player) {
  ...
  const target = player === 1 ? logBlackElem : logWhiteElem;
...
function startGame() {
  ...
  initBoard();
  renderBoard();
  logBlackElem.textContent = '';
  logWhiteElem.textContent = '';
  resultPopup.classList.add('hidden');
  resultPopup.textContent = '';
  if (mode === 'ai-human' && currentPlayer === 1) {
    aiTurn(1);
